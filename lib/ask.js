...
const viewEmployees = () => {
  connection.query(
    "SELECT employees.id, employees.first_name, employees.last_name, employees.emp_role, roles.salary, roles.department FROM employees INNER JOIN roles ON employees.emp_role = roles.title",
    (err, res) => {
      if (err) throw err;
